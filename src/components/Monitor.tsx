...
/** As 3 opções do seletor de resolução, na ordem em que aparecem — da mais
 *  nítida (mais pesada) à mais leve, como no seletor de resolução de
 *  reprodução do Premiere. Vive no Monitor (não nas Configurações): é um
 *  controle de VISUALIZAÇÃO, de efeito imediato, não uma preferência atrás
 *  de um "Salvar". */
const RESOLUTION_OPTIONS: {
  value: proxy.Resolution;
  labelKey: "resolutionFull" | "resolutionHalf" | "resolutionQuarter";
}[] = [
  { value: "full", labelKey: "resolutionFull" },
  { value: "half", labelKey: "resolutionHalf" },
  { value: "quarter", labelKey: "resolutionQuarter" },
];
...
export function Monitor() {
  ...
  const {
    ...
    setPlaybackResolution: setResolution,
  } = useAppStore();
  ...
  /** Resolução de reprodução do proxy (teto FHD) — carga inicial do que está
   *  salvo em disco; o seletor no rodapé deste componente (`changeResolution`)
   *  cuida das trocas seguintes direto na store, sem remontar nada. */
  useEffect(() => {
  ...
  /** A prévia ATUAL está no proxy (não no original nativo)? Só nesse caso uma
   *  troca de resolução tem algo a regenerar — resolução não significa nada
   *  pro caminho nativo. Decidido uma vez por clipe (`goProxy`), lido pelo
   *  efeito de troca de resolução, que NÃO depende do clipe (só de `resolution`). */
  const previewUsesProxyRef = useRef(false);
  ...
  /** Muda a resolução AGORA (reflete na store — o efeito acima que gera o
   *  proxy já depende de `resolution`) e persiste em segundo plano; falhar
   *  a escrita em disco não deve travar a troca que o usuário já viu na tela. */
  const changeResolution = (r: proxy.Resolution) => {
    setResolution(r);
    void invoke("sidecar_call", {
      command: "set_settings",
      params: { playback_resolution: r },
    }).catch(() => {});
  };
   ...
   *
   * DEPENDE só de `previewClip`, de propósito — NÃO de `resolution`. Este efeito
   * é a carga DO ZERO de um clipe (`v.src = ...; v.load()`), e reagir a
   * `resolution` aqui reiniciava a prévia do zero a cada troca (o vídeo voltava
   * pro início). Uma troca de resolução com a prévia já tocando é tratada pelo
   * efeito seguinte, que REGENERA o proxy preservando posição — resolução não
   * pede um novo `load()` do original nativo, só um proxy diferente.
   */
  useEffect(() => {
    ...
    previewUsesProxyRef.current = false;
    if (!previewClip) {
      v.pause();
      v.removeAttribute("src");
      v.load();
    ...
    const goProxy = async () => {
      if (cancelled) return;
      previewUsesProxyRef.current = true;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `resolution` de
    // propósito fora: ver o efeito seguinte.
  }, [previewClip]);

  /**
   * Troca de resolução com a prévia JÁ tocando: só faz sentido regenerar se a
   * prévia atual estiver no PROXY (`previewUsesProxyRef`) — o caminho nativo
   * ignora `resolution` por completo. Preserva posição e estado de play/pause;
   * falhar aqui mantém o proxy anterior tocando (trocar resolução é um extra,
   * nunca pode derrubar uma prévia que já funcionava).
   */
  useEffect(() => {
    const v = previewVideoRef.current;
    if (!v || !previewClip || !previewUsesProxyRef.current) return;
    const clip = previewClip;
    let cancelled = false;
    const wasPlaying = !v.paused;
    const posSec = v.currentTime;
    ...
    const regen = async () => {
      setPreviewCaching(true);
      try {
        const proxyPath = await proxy.preview(clip.path, resolution);
        if (cancelled) return;
        v.addEventListener("loadeddata", onReady, { once: true });
        v.src = convertFileSrc(proxyPath);
        v.load();
      } catch {
        /* mantém o proxy anterior tocando — ver o comentário acima */
      } finally {
        if (!cancelled) setPreviewCaching(false);
      }
    };
    void regen();
    ...
    <div className="flex flex-col h-full min-w-0 bg-surface">
      ...
      <div className="flex-shrink-0 px-3 pb-3 flex flex-col gap-2">
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <select
            className="ml-auto text-[11px] bg-surface-2 border border-line rounded px-1.5 py-0.5 text-ink-2 outline-none focus:border-accent flex-shrink-0"
            value={resolution}
            onChange={(e) => changeResolution(e.target.value as proxy.Resolution)}
            title={t.monitor.resolutionHint}
          >
            {RESOLUTION_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value}>
                {t.monitor[opt.labelKey]}
              </option>
            ))}
          </select>
        </div>
