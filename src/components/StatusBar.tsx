...
export function StatusBar() {
  ...
  // A versão no rodapé é o único lugar onde o resultado da checagem aparece —
  // sem popup novo, de propósito (o usuário não quer nada distrativo). 3
  // estados possíveis: em dia/checando (cor padrão, texto simples), atrasado
  // (vermelho + ícone + vira link pro release) e falha de checagem (amarelo —
  // "não sei se está em dia" é visualmente diferente de "está em dia").
  const versionText = t.footer.version.replace("{{version}}", APP_VERSION);
  let versionColorClass = "text-ink-3";
  let versionTitle: string | undefined;
  let versionUrl: string | undefined;
  if (update?.kind === "outdated") {
    versionColorClass = "text-ink-danger";
    versionTitle = t.footer.updateAvailableHint;
    versionUrl = update.url;
  } else if (update?.kind === "error") {
    versionColorClass = "text-ink-warning";
    versionTitle = t.footer.updateCheckFailedHint;
  }
  ...
  return (
    <div className="flex items-center gap-3 px-4 h-7 bg-surface-2 border-t border-line flex-shrink-0">
      ...
      <div className="ml-auto flex items-center gap-3 flex-shrink-0">
        ...
        {versionUrl ? (
          <button
            className={`text-[11px] flex items-center gap-1 underline-offset-2 hover:underline ${versionColorClass}`}
            onClick={() => void openUrl(versionUrl!)}
            title={versionTitle}
          >
          ...
          <span className={`text-[11px] ${versionColorClass}`} title={versionTitle}>
            {versionText}
          </span>
        )}
        <Signature />
