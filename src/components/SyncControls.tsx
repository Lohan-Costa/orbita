...
export function SyncControls() {
  ...
  const canSync = stale.length > 0 && readyClips.length >= 2 && !isRunning;
  const isStale =
  ...
  return (
    <div className="flex-shrink-0 border-t border-line bg-surface-2 px-4 py-3">
      <div className="flex items-center gap-2">
        {isStale && (
          <span className="status-pill pill-warning">{t.timeline.stale}</span>
        )}
        <button
          className="tbtn outline-accent gap-1.5 ml-auto"
          onClick={handleSync}
          disabled={!canSync}
        >
