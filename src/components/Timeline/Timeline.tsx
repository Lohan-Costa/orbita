...
export function Timeline({ height: panelHeight }: TimelineProps) {
  ...
  /** Multi-seleção. O `anchor` é de onde um shift-clique mede o intervalo.
   *  Declarada aqui em cima porque a pré-visualização do arrasto (`data`, logo
   *  abaixo) precisa saber QUEM está selecionado — todos se movem juntos. */
   ...
   * selecionados. O som não tem offset próprio para recalcular (ver
   * [[manual-correction-model]]), então uma seleção que só tem som não oferece o
   * botão. O resto da diária entra FIXADO no que já está na tela — inclusive
   * correções manuais — e serve de ÂNCORA: é o "vizinho confiável" que resolve uma
   * tomada ambígua sem reler um arquivo que ninguém pediu para reconsiderar (ver
   * `pinned`/`selected` em `sync/engine.py`).
   */
  ...
  const handleResyncSelection = async () => {
    ...
    for (const cg of result.camera_groups) {
      for (const c of cg.cameras) {
        ...
        pinned[c.path] = { offset_frames: c.sync_offset_frames, sound_path: take.sound.path };
      }
    }
    ...
    try {
      const response = await invoke<{
        ...
        params: {
          ...
          // O MESMO método do sync geral (dropdown): o usuário troca para "Waveform"
          // para consertar por forma de onda os poucos clipes onde o TC falhou, ou
          // mantém "Timecode" para reaplicar o TC de entrada à seleção.
          ...
          // deixaria pin e medição em unidades diferentes. (Trocar o fps do projeto
          // já marca o resultado como obsoleto — o caminho certo ali é o sync
          // cheio, não o parcial.)
          fps: result.fps,
  ...
  return (
    ...
    >
      {/* Toolbar */}
      <div className="flex items-center gap-2 px-4 h-8 border-b border-line flex-shrink-0">
        <span className="panel-title">{t.timeline.title}</span>
        ...
        <span
          ref={playheadTcRef}
          className="text-[11px] font-mono tabular-nums text-ink-2 w-[86px]"
        />
        ...
        {data && (
          <>
            ...
            {flaggedCount > 0 && (
              <span className="status-pill pill-warning gap-1">
            ...
            {peaksProgress && (
              <span className="status-pill pill-muted gap-1" title={t.timeline.peaksHint}>
        ...
        {syncResult && selected.size === 0 && !timelineLocked && (
          <span className="text-[11px] text-ink-3 ml-2">{t.timeline.editHint}</span>
        )}
        ...
        {selectedClip && data && (
          <span className="text-[11px] text-ink-2 font-mono ml-2">
        ...
        {selectedClips.length > 1 && (
          <span className="status-pill pill-muted ml-2">
            {t.timeline.selectedCount.replace("{{n}}", String(selectedClips.length))}
          </span>
        )}

        {/* RE-SYNC PARCIAL (Etapa D) — só quando há câmera na seleção: o som não
            tem offset próprio para recalcular. */}
        ...
        {/* Recortar uma cena da seleção. Não depende da TRAVA: criar uma cena não
            move clipe nenhum — é uma vista, e ver não é editar. */}
        {syncResult && selectedClips.length > 0 && (
          <button
            className="tbtn gap-1"
            onClick={() => {
              ...
              addSubGroup(
                activeGroupId,
                t.groups.subGroupDefault.replace("{{n}}", String(subGroups.length + 1)),
        ...
        {/* Confirmar/reverter valem para a seleção INTEIRA. */}
        {canEdit && selectedClips.some((c) => c.editable) && (
          <div className="flex items-center gap-1">
            {selectedClips.some(
              (c) => c.editable && (c.flagged || c.manuallyAdjusted) && !c.confirmed
            ) && (
              <button
                className="tbtn gap-1"
                // Em LOTE, e não num laço aqui: o store é quem sabe em quem agir e
                // em que ordem — ver `confirmClips`/`revertClips`.
                onClick={() =>
                  confirmClips(
                    selectedClips.filter((c) => c.editable).map((c) => c.path)
                  )
                }
            ...
            {selectedClips.some(
              (c) => c.editable && (c.manuallyAdjusted || c.confirmed)
            ) && (
              <button
                className="tbtn gap-1"
                onClick={() =>
            ...
            {/* "Ao original" só aparece quando há um MARCO para pular por baixo —
                sem ele, os dois botões fariam a mesma coisa, e dois botões iguais só
                ensinam o usuário a não ler o que está escrito. */}
            {selectedClips.some((c) => c.editable && c.hasCheckpoint) && (
              <button
                className="tbtn gap-1"
                onClick={() =>
                  revertClipsToOriginal(
