...
import type { Resolution } from "../lib/proxy";
...
interface AppState {
  ...
  /** Resolução de reprodução do proxy do monitor/prévia (teto Full HD; `half`/
   *  `quarter` são relativos a esse teto, não ao arquivo — ver media/proxy.py).
   *  Cópia em memória do que está salvo em `settings.json` pelo sidecar: o
   *  Monitor já fica montado a sessão inteira, então precisa de um valor
   *  reativo aqui para aplicar uma mudança feita nas Configurações NA HORA,
   *  sem remontar. */
  playbackResolution: Resolution;
  setPlaybackResolution: (r: Resolution) => void;
  ...
  /**
   * Os clipes SELECIONADOS na timeline (por path).
   *
   * Mora na store, e não no estado local da timeline, porque as AÇÕES sobre a
   * seleção (confirmar, reverter, re-sincronizar, criar cena) vivem na barra de
   * baixo, junto do Sincronizar/Exportar — outro componente. Duas cópias da
   * seleção seriam duas que um dia divergem: a barra agindo sobre um conjunto
   * diferente do que está aceso na tela.
   */
  timelineSelection: Set<string>;
  setTimelineSelection: (paths: Set<string>) => void;

  /**
   * Progresso do LOTE de importação em curso (leitura de metadados), ou `null`.
   *
   * Precisa ser contado explicitamente porque a alternativa — deduzir de
   * `clips` — mede o PROJETO inteiro: com 50 clipes já prontos e 10 entrando, a
   * conta dava "50 de 60", a barra nascia em 83% e mal se movia, parecendo um
   * app travado perto do fim. E clipes de OUTRAS diárias entravam na conta.
   */
  importProgress: { done: number; total: number } | null;
  setImportProgress: (p: { done: number; total: number } | null) => void;

  /** Prévia solta de um clipe do painel de conteúdo (duplo-clique), fora da
   *  timeline sincronizada — só para OLHAR o arquivo cru, câmera ou som. Não
   *  mexe no motor de sync/transporte; o Monitor a desenha por cima da tela
   *  normal e sai sozinha ao dar play no transporte principal. */
  previewClip: { path: string; name: string } | null;
  setPreviewClip: (clip: { path: string; name: string } | null) => void;
  ...
  /**
   * FECHAR LACUNAS — encosta as tomadas umas nas outras, sem buracos e sem
   * sobreposição, na diária ATIVA.
   *
   * A unidade que se move é a TOMADA inteira (som + câmeras dela), pelo mesmo Δ:
   * o `sync_offset` sobrevive e NADA fica roxo. Ver `closeGapsIn`.
   */
  closeGaps: () => void;
  ...
  /** `kind` é o PALPITE inicial (o Rust classifica por extensão no import). Não
   *  trava nada: o probe ainda corrige, e o usuário por cima dele. */
  addSource: (
    groupId: string,
    name: string,
    folderPath?: string,
  ...
  /**
   * Move clipes para outra FONTE da mesma diária — o conserto manual de quando o
   * palpite de "quantas câmeras há nesta pasta" erra (ver `plan_split` no Rust).
   *
   * É o recurso do usuário contra a heurística: importou a câmera e o drone
   * juntos, seleciona os do drone e os joga numa câmera nova.
   *
   * NÃO atravessa diárias: um arquivo vive numa só, e mover entre elas mudaria a
   * qual som ele se sincroniza — outra coisa, não um "mover".
   */
  moveClipsToSource: (groupId: string, targetSourceId: string, paths: string[]) => void;
...
/**
 * O que se move INTEIRO ao fechar lacunas: uma tomada (o som mais as câmeras
 * dela) ou um clipe solto.
 *
 * A unidade é a TOMADA, e não o clipe, porque mover câmera e som separadamente
 * destruiria o sync — que é a única coisa que este app existe para acertar.
 */
interface TimelineUnit {
  startFrames: number;
  endFrames: number;
  /** Os paths que andam juntos (câmeras + o som, quando é tomada). */
  paths: string[];
}

/** Duração de uma câmera NA GRADE DO PROJETO.
 *
 *  ⚠️ `duration_frames` vem na grade da PRÓPRIA câmera (ver `assemble`), enquanto
 *  `timeline_start_frames` está na do projeto. Somar os dois sem converter erra
 *  em qualquer diária com câmera de fps diferente. */
function cameraDurationInProjectFrames(c: SyncedCamera, fps: number): number {
  return Math.round((c.duration_frames / (c.fps || fps)) * fps);
}

function soundDurationInFrames(s: SyncedSound, fps: number): number {
  return Math.round((s.duration_ms / 1000) * fps);
}

/** As unidades da timeline, na ordem em que aparecem. */
function timelineUnits(result: SyncResult): TimelineUnit[] {
  const fps = result.fps;
  const camByPath = new Map(
    result.camera_groups.flatMap((g) => g.cameras.map((c) => [c.path, c] as const))
  );
  const inTake = new Set(result.takes.flatMap((t) => t.camera_paths));
  const units: TimelineUnit[] = [];

  for (const take of result.takes) {
    const cams = take.camera_paths
      .map((p) => camByPath.get(p))
      .filter((c): c is SyncedCamera => !!c);
    const starts = [take.sound.timeline_start_frames, ...cams.map((c) => c.timeline_start_frames)];
    const ends = [
      take.sound.timeline_start_frames + soundDurationInFrames(take.sound, fps),
      ...cams.map((c) => c.timeline_start_frames + cameraDurationInProjectFrames(c, fps)),
    ];
    units.push({
      startFrames: Math.min(...starts),
      endFrames: Math.max(...ends),
      paths: [take.sound.path, ...cams.map((c) => c.path)],
    });
  }

  // Os SOLTOS também entram: deixá-los parados enquanto as tomadas se juntam os
  // faria colidir com o que passou a ocupar aquele tempo — e o pedido é "sem
  // sobrescrever nada".
  for (const [path, c] of camByPath) {
    if (inTake.has(path)) continue;
    units.push({
      startFrames: c.timeline_start_frames,
      endFrames: c.timeline_start_frames + cameraDurationInProjectFrames(c, fps),
      paths: [path],
    });
  }
  for (const s of result.orphan_sounds ?? []) {
    units.push({
      startFrames: s.timeline_start_frames,
      endFrames: s.timeline_start_frames + soundDurationInFrames(s, fps),
      paths: [s.path],
    });
  }

  return units.sort((a, b) => a.startFrames - b.startFrames);
}

/**
 * Aplica um deslocamento por path, mexendo SÓ na posição.
 *
 * ⚠️ O `checkpoint_start` (o marco de um ÓRFÃO) anda JUNTO, e isso é
 * load-bearing. Um órfão não tem som contra o que medir, então a grandeza que
 * define o sync dele é a POSIÇÃO (ver `reconcileAdjusted.syncOf`). Deixando o
 * marco parado enquanto o clipe se move, `reconcileAdjusted` conclui que o sync
 * mudou e APAGA a confirmação do usuário — uma revisão perdida em silêncio.
 *
 * O `checkpoint_offset` (o marco de quem está numa tomada) NÃO se mexe: ele
 * guarda uma relação, e a relação sobrevive ao deslocamento em lote.
 */
function shiftByPath(result: SyncResult, delta: Map<string, number>): SyncResult {
  const move = (start: number, path: string) => start + (delta.get(path) ?? 0);
  ...
  return {
    ...
    camera_groups: result.camera_groups.map((g) => ({
      ...
      cameras: g.cameras.map((c) =>
        delta.has(c.path)
          ? {
              ...c,
              timeline_start_frames: move(c.timeline_start_frames, c.path),
              ...(c.checkpoint_start !== undefined
                ? { checkpoint_start: move(c.checkpoint_start, c.path) }
                : {}),
            }
          : c
      ),
    ...
    takes: result.takes.map((t) =>
      delta.has(t.sound.path)
        ? {
            ...t,
            sound: {
              ...t.sound,
              timeline_start_frames: move(t.sound.timeline_start_frames, t.sound.path),
            },
          }
        : t
    ),
    orphan_sounds: result.orphan_sounds?.map((s) =>
      delta.has(s.path)
        ? { ...s, timeline_start_frames: move(s.timeline_start_frames, s.path) }
        : s
    ),
  };
}
...
/**
 * Encosta as unidades umas nas outras, matando os buracos entre elas.
 *
 * ⚠️ O QUE ESTA FUNÇÃO NÃO FAZ, e é o ponto: ela não toca em
 * `sync_offset_frames` nem acende `manually_adjusted`. Câmera e som da mesma
 * tomada andam o MESMO Δ, e `(cam+Δ) − (snd+Δ) = cam − snd` — o sync sobrevive
 * intacto. Deslocar a tomada inteira não é correção de sync; acender o roxo aqui
 * anunciaria uma correção que não existe e ensinaria o usuário a desconfiar do
 * aviso quando ele importa.
 *
 * A PRIMEIRA unidade fica onde está: o buraco no começo é a distância até o
 * início do projeto, e engoli-lo mudaria o timecode de tudo — outra operação,
 * que ninguém pediu.
 */
function closeGapsPlan(result: SyncResult): Map<string, number> {
  const units = timelineUnits(result);
  const delta = new Map<string, number>();
  if (units.length < 2) return delta;

  let cursor = units[0].endFrames;
  for (const u of units.slice(1)) {
    const d = cursor - u.startFrames;
    if (d !== 0) for (const p of u.paths) delta.set(p, d);
    // O fim é medido DEPOIS do deslocamento — senão as unidades seguintes se
    // encostariam nas posições antigas e voltariam a sobrepor.
    cursor = u.endFrames + d;
  }
  return delta;
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setPlaybackResolution: (playbackResolution) => set({ playbackResolution }),
  ...
  timelineSelection: new Set<string>(),
  ...
  /**
   * ⚠️ NÃO passa pelo `mapActive`, e é de propósito: o deslocamento em lote tem
   * de mover o BASELINE junto com o resultado (mesmo padrão do
   * `applyResyncResult`).
   *
   * O baseline é "onde o algoritmo pôs cada coisa". Para um clipe ÓRFÃO essa
   * referência é a POSIÇÃO — e deixá-la parada enquanto o clipe se move faria
   * `reconcileAdjusted` concluir que o sync mudou, apagando a confirmação do
   * usuário e acendendo o roxo num clipe que ninguém dessincronizou.
   */
  closeGaps: () =>
    set((s) => {
      const i = s.syncGroups.findIndex((g) => g.id === s.activeGroupId);
      ...
      const delta = closeGapsPlan(group.result);
      if (delta.size === 0) return {};   // já estava tudo encostado

      const shifted = shiftByPath(group.result, delta);
      const baseline = group.baseline ? shiftByPath(group.baseline, delta) : null;
      const result = reconcileAdjusted(shifted, baseline);
  ...
  addSource: (groupId, name, folderPath, kind = "camera") => {
    const id = crypto.randomUUID();
    set((s) => ({
      syncGroups: s.syncGroups.map((g) =>
        g.id === groupId
          ? {
              ...g,
              // Só o palpite inicial: o probe decide, e o usuário corrige.
              sources: [...g.sources, { id, name, kind, folderPath }],
            }
          : g
      ),
    }));
  ...
  moveClipsToSource: (groupId, targetSourceId, paths) =>
    set((s) => {
      const moving = new Set(paths);
      const target = s.syncGroups
        .find((g) => g.id === groupId)
        ?.sources.find((src) => src.id === targetSourceId);
      if (!target) return {};

      // A ordem dentro da fonte de DESTINO continua depois do que já está lá —
      // o `sourceOrder` é a ordem de gravação, e o engine a usa para desempatar
      // quando o timecode não serve (ver sync/engine.py::_natural_sort_key).
      let next = s.clips
        .filter((c) => c.sourceId === targetSourceId)
        .reduce((m, c) => Math.max(m, c.sourceOrder + 1), 0);

      return {
        clips: s.clips.map((c) =>
          c.syncGroupId === groupId && moving.has(c.path) && c.sourceId !== targetSourceId
            ? { ...c, sourceId: targetSourceId, sourceOrder: next++ }
            : c
        ),
      };
    }),
