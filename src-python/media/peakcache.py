...
ONDE: no diretório de cache do usuário, NÃO junto da mídia. O material bruto pode
estar num cartão, num HDD só de leitura ou num NAS compartilhado — escrever ao lado
dele é presunçoso e às vezes impossível.

  macOS   ~/Library/Caches/Orbita/peaks
  Windows %LOCALAPPDATA%\\Orbita\\Cache\\peaks
  Linux   ~/.cache/Orbita/peaks
...
def load(path: str | Path) -> np.ndarray | None:
    ...
    if f is None or not f.is_file():
        return None
    try:
        return np.frombuffer(f.read_bytes(), dtype=np.uint8)
    except OSError:
        return None
