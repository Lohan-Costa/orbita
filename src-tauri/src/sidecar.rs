    ...
    pub fn detect(manifest_dir: &str) -> Self {
        ...
        if let Ok(exe) = std::env::current_exe().and_then(std::fs::canonicalize) {
            if let Some(dir) = exe.parent() {
                let bin = dir.join("orbita-python");
                if bin.exists() {
                    return Self::new(bin.to_string_lossy().to_string(), vec![]);
                }
            }
        }

        // 2. Venv de desenvolvimento: src-python/.venv/bin/python + src-python/main.py
