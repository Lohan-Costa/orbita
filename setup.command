...
if ! command -v brew >/dev/null 2>&1; then
  echo "[1/6] Homebrew não encontrado. Vou instalar."
  echo "      >> Pode pedir a SENHA do seu login do Mac. É normal — digite e pressione Enter."
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
  [ -x /opt/homebrew/bin/brew ] && eval "$(/opt/homebrew/bin/brew shellenv)"
  [ -x /usr/local/bin/brew ]    && eval "$(/usr/local/bin/brew shellenv)"
else
  echo "[1/6] Homebrew já instalado. Ok."
fi

# --- 2/6 Ferramentas de linha de comando do Xcode ---
if ! xcode-select -p >/dev/null 2>&1; then
  echo "[2/6] Ferramentas do Xcode não encontradas. Vou pedir a instalação."
  echo "      >> Vai abrir uma janela do sistema. Clique em 'Instalar' e aguarde."
  xcode-select --install
  echo "      Aguardando a instalação terminar..."
  until xcode-select -p >/dev/null 2>&1; do sleep 5; done
else
  echo "[2/6] Ferramentas do Xcode já instaladas. Ok."
fi

# --- 3/6 Rust ---
if ! command -v cargo >/dev/null 2>&1; then
  echo "[3/6] Rust não encontrado. Vou instalar."
  curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y
  . "$HOME/.cargo/env"
else
  echo "[3/6] Rust já instalado. Ok."
  . "$HOME/.cargo/env" 2>/dev/null || true
fi

# --- 4/6 Node.js ---
if ! command -v node >/dev/null 2>&1; then
  echo "[4/6] Node.js não encontrado. Vou instalar via Homebrew."
  brew install node
else
  echo "[4/6] Node.js já instalado. Ok."
fi

# --- 5/6 Python (necessário para construir o sidecar) ---
if ! command -v python3 >/dev/null 2>&1; then
  echo "[5/6] Python não encontrado. Vou instalar via Homebrew."
  brew install python
else
  echo "[5/6] Python já instalado. Ok."
fi
